#połączyłam start i samą grę, ale po naciśnięciu przycisku SuperMariusz przestaje chodzić 
#i niestety nie mam idei jak to naprawić
...
def setup():
    size(1000, 500)
    global value, clr, klucz, drzwi, pos_x, pos_y, sm_Stand, sm_Walk, sm_Jump, is_walk, is_flipped, sm_StandFlipped, sm_WalkFlipped, sm_JumpFlipped
    text_size = 40
    textSize(text_size)
    value = 255
    clr = 127  
...
def draw():
    global value, clr, pos_x, pos_y, sm_Stand, sm_Walk, is_walk, is_flipped, sm_StandFlipped, sm_WalkFlipped, sm_JumpFlipped, victory, na_drzwiach
    background(0)
    fill(value)
    rect(290, 330, 250, 100)
    text("       WELCOME \nTO SUPERMARIUSZ!",width/4-40, height/3)
    fill(clr, 0, 0)
    text("START", 360, 395)
    if mousePressed == True:   
        if mouseX>=290 and mouseX<=540 and mouseY>=330 and mouseY<=430: 
            noLoop()
            background(0) 
            fill(30, 200, 0)
            rect(375, 275, 175, 15)
